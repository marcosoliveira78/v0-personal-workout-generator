        ...
        <div className="mt-4 space-y-6 print:p-0">
          ...
          <div>
            ...
            <div className="space-y-6">
              {currentWeek.workouts.map((workout, index) => (
                <div key={`workout-${index}-${workout.id || workout.name}`} className="border p-4 rounded-md">
                    ...
                    <table className="min-w-full text-sm">
                      ...
                      <tbody className="divide-y">
                        {workout.exercises.map((exercise, i) => (
                          <tr
                            key={`exercise-${index}-${i}-${exercise.name}`}
          ...
          {currentWeek.restDayActivities.length > 0 && (
            <div>
              ...
              <div className="space-y-4">
                {currentWeek.restDayActivities.map((activity, activityIndex) => (
                  <div
                    key={`rest-activity-${activityIndex}-${activity.name}`}
                    ...
                    <div className="mt-2">
                      ...
                      <ul className="list-disc list-inside text-sm">
                        {activity.benefits.map((benefit, i) => (
                          <li key={`benefit-${activityIndex}-${i}`}>{benefit}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
          ...
          {workoutPlan.supplementRecommendations && workoutPlan.supplementRecommendations.length > 0 && (
            <div>
              ...
              <div className="space-y-4">
                {workoutPlan.supplementRecommendations.map((supplement, suppIndex) => (
                  <div key={`supplement-${suppIndex}-${supplement.name}`} className="border p-4 rounded-md">
                    ...
                    <div className="mt-2">
                      ...
                      <ul className="list-disc list-inside text-sm">
                        {supplement.benefits.map((benefit, i) => (
                          <li key={`supp-benefit-${suppIndex}-${i}`}>{benefit}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
          ...
          {workoutPlan.sleepRecommendations && workoutPlan.sleepRecommendations.length > 0 && (
            <div>
              ...
              <ul className="list-disc list-inside space-y-1 bg-blue-50 p-4 rounded-md">
                {workoutPlan.sleepRecommendations.map((rec, sleepIndex) => (
                  <li key={`sleep-rec-${sleepIndex}`} className="text-sm">
                    {rec}
                  </li>
                ))}
              </ul>
