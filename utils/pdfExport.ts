...
export async function exportWorkoutPlanToPDF(workoutPlan: WorkoutPlan): Promise<void> {
  try {
    // Carregando as bibliotecas dinamicamente apenas no cliente
    ...
    doc.setProperties({
      title: workoutPlan.name || "Plano de Treino",
    ...
    // Adicionar título
    doc.setFontSize(20)
    doc.text(workoutPlan.name || "Plano de Treino Personalizado", 14, 22)
    ...
    // Salvar o PDF
    const fileName = `plano-treino-${(workoutPlan.name || "personalizado").replace(/\s+/g, "-").toLowerCase()}.pdf`
    doc.save(fileName)
